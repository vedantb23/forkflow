...
| **DB access** | Raw SQL via `pg` driver (no ORM) | Direct control over queries; schema in plain `.sql` |
...
│   ├── Dockerfile.worker
│   ├── db/
│   │   ├── schema.sql               # all tables + enums (raw SQL)
│   │   ├── migrate.ts               # runs schema.sql on the DB
...
│       │   ├── db.ts                # pg Pool (shared Postgres client)
...
## 🗄️ Core Data Model (high level — full schema in `db/schema.sql`, built Day 1)
...
# 3. Start infra (Redis) + set up DB
docker compose up -d redis
cd backend
npm run db:migrate          # creates tables in Supabase (runs db/schema.sql)
...
| Day | Focus | Outcome |
|-----|-------|---------|
| **1** | Foundation & Infra | Repo scaffold, Docker, **Supabase setup**, SQL schema + migrate, Redis connected |
